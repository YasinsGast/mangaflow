...
export default function DashboardPage() {
  ...
  const [readingListMangas, setReadingListMangas] = useState<any[]>([]);
  ...
  useEffect(() => {
    const loadReadingList = async () => {
      console.log('[DashboardPage] Loading reading list...');
      setReadingListLoading(true);
      const readingListData = await getReadingList();
      console.log('[DashboardPage] Reading list loaded:', readingListData.length);
      setReadingListMangas(readingListData);
  ...
  const stats = [
    { label: 'Okunan Bölüm', value: bookmarks.length.toString(), icon: Book, color: 'from-blue-500 to-cyan-500', bgColor: 'bg-blue-500/10' },
    { label: 'Favoriler', value: favorites.length.toString(), icon: Heart, color: 'from-pink-500 to-rose-500', bgColor: 'bg-pink-500/10' },
    { label: 'Okuma Listesi', value: readingListMangas.length.toString(), icon: Bookmark, color: 'from-purple-500 to-violet-500', bgColor: 'bg-purple-500/10' },
    { label: 'Toplam Süre', value: '0s', icon: Clock, color: 'from-orange-500 to-amber-500', bgColor: 'bg-orange-500/10' },
  ];
        ...
        >
          ...
          {favoritesLoading ? (
            ...
            >
              ...
              <p className="text-gray-400 text-lg mb-2">Henüz favori manga eklemediniz</p>
              <p className="text-gray-500 text-sm mb-6">Sevdiğiniz mangaları favorilere ekleyerek buradan kolayca ulaşabilirsiniz</p>
              <Link to="/library">
                <motion.button
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
        ...
        >
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-2xl font-bold text-white">Devam Eden Okumalar</h2>
            </div>
          </div>
          ...
          {bookmarksLoading ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-6">
              {(() => {
                console.log('[DashboardPage] RENDERING BOOKMARKS GRID!', {
            ...
            >
              ...
              <p className="text-gray-400 text-lg mb-2">Devam eden okumanız yok</p>
              <p className="text-gray-500 text-sm mb-6">Okumaya devam etmek istediğiniz mangalar burada görünecek</p>
              <Link to="/library">
                <motion.button
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
        ...
        >
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-2xl font-bold text-white">Okuma Listem</h2>
            </div>
            {readingListMangas.length > 0 && (
              <Link to="/library">
                <motion.button
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
          ...
          {readingListLoading ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-6">
              {readingListMangas.slice(0, 6).map((manga, idx) => (
                <ReadingListMangaCard key={manga.id} manga={manga} index={idx} />
              ))}
            </div>
          ) : (
            <div className="backdrop-blur-md rounded-2xl p-8 border border-purple-500/20 text-center"
            ...
            >
              <Bookmark className="h-16 w-16 mx-auto mb-4 text-gray-600" />
              <p className="text-gray-400 text-lg mb-2">Henüz okuma listeniz yok</p>
              <p className="text-gray-500 text-sm mb-6">Okumak istediğiniz mangaları listeye ekleyerek buradan kolayca ulaşabilirsiniz</p>
              <Link to="/library">
                <motion.button
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                  className="px-6 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-semibold rounded-xl shadow-lg hover:shadow-indigo-500/50 transition-all"
                >
                  Manga Keşfet
                </motion.button>
              </Link>
...
function FavoriteMangaCard({ manga, index }: { manga: any; index: number }) {
  return (
    <MangaCard
      manga={manga}
      index={index}
      type="favorite"
      badge={manga.rating_average ? {
        text: manga.rating_average.toFixed(1),
...
// Standart Manga Card Component
interface MangaCardProps {
  manga: any;
  index: number;
  type: 'favorite' | 'reading-list' | 'bookmark';
  badge?: {
    text: string;
    color: string;
    icon: any;
...
function MangaCard({ manga, index, type, badge, meta }: MangaCardProps) {
  const cardRef = useRef(null);
  const isInView = useInView(cardRef, { once: true, amount: 0.3 });

  // URL belirleme
  const getUrl = () => {
    if (type === 'bookmark' && meta?.chapter && meta?.page) {
      return `/read/${manga.slug || manga.manga_slug}/${meta.chapter}?page=${meta.page}`;
    }
    return `/manga/${manga.slug || manga.manga_slug}`;
  };

  // Başlık belirleme
  const getTitle = () => {
    return manga.title || manga.manga_title || 'Bilinmeyen Manga';
  };

  // Kapak görsel belirleme
  const getCoverImage = () => {
    return manga.cover_image_url || manga.manga_cover_url || 'https://images.unsplash.com/photo-1618519764620-7403abdbdfe9?w=400&h=600&fit=crop';
  };

  // Bölüm sayısı belirleme
  const getChapterInfo = () => {
    return manga.total_chapters || `${meta?.chapter || 0} bölüm`;
  };

  // Badge rengi belirleme
  const getHoverColor = () => {
    const colors = {
      'favorite': 'hover:text-pink-300',
      'reading-list': 'hover:text-indigo-300',
      'bookmark': 'hover:text-blue-300'
    };
    return colors[type];
  };
      ...
      <Link to={getUrl()}>
        ...
        >
          ...
          <div className="relative aspect-[3/4] overflow-hidden">
            ...
            {/* Badge */}
            {badge && (
              <div className="absolute top-2 right-2 px-2 py-1 rounded-lg backdrop-blur-md" style={{ background: badge.bgColor, border: '1px solid rgba(255, 255, 255, 0.2)' }}>
            ...
            {/* Progress Info - Bookmark için */}
            {type === 'bookmark' && meta?.chapter && meta?.page && (
          ...
          {/* Title */}
          <div className="p-3">
            <h3 className={`text-white font-semibold text-sm line-clamp-2 transition-colors ${getHoverColor()}`}>
              {getTitle()}
            </h3>
            <div className="flex items-center justify-between mt-1">
              <p className="text-slate-400 text-xs">{getChapterInfo()}</p>
              {meta?.date && (
...
// Reading List Manga Card Component
function ReadingListMangaCard({ manga, index }: { manga: any; index: number }) {
  return (
    <MangaCard
      manga={manga}
      index={index}
  ...
  return (
    <MangaCard
      manga={{
        ...
        cover_image_url: bookmark.manga_cover_url
      }}
      index={index}
      type="bookmark"
      badge={{
      ...
      meta={{
        title: bookmark.manga_title || 'Bilinmeyen Manga',
